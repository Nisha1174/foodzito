    ...
    const handleManualSubmit = async () => {
        ...
        try {
            ...
            if (data.length > 0) {
                ...
                const name = manualAddress
                const city = data[0].display_name.split(',')[1]?.trim() || ''
                dispatch(setLocation({ coords, name, city }))
                onClose()
            }
        } catch {
        ...
        <>
            ...
            <div className="animate-scaleIn" style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                    <div>
                        <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: '700', fontSize: '18px', color: '#0f0f0f', margin: 0 }}>
                            📍 Set Your Location
                        </h3>
                ...
                {/* Detect GPS */}
                <button onClick={handleDetectLocation} disabled={detecting} style={{
                ...
                {/* Popular Cities */}
                <p style={{ margin: '0 0 10px', fontSize: '12px', fontWeight: '600', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    Popular Cities
                </p>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
                    {popularCities.map(city => (
                        <button key={city.name} onClick={() => handleCitySelect(city)} style={{
                            padding: '10px 8px',
                ...
                {locationName && (
                    <button onClick={() => { dispatch(clearLocation()); onClose() }} style={{
